...
namespace ACADTRANSFORMER.THUVIENCAD
{
    partial class MyAcad
    {
                ...
                using (DocumentLock acLockDoc = acDoc.LockDocument())
                {
                    using (Transaction acTrans = acCurDB.TransactionManager.StartTransaction())
                    {
                        ...
                        using (RotatedDimension acRotDim = new RotatedDimension()) //Dimlinear C
                        {
                            ...
                            acRotDim.DimensionStyle = acDimstyleTbl["X26.4_BTK"];
                        ...
                        using (LineAngularDimension2 acAngular = new LineAngularDimension2()) // Ve kich thuoc goc 90 do
                        {
                            ...
                            acAngular.DimensionStyle = acDimstyleTbl["X26.4_BTK"];
                    ...
                    using (Transaction acTrans = acCurDB.TransactionManager.StartTransaction())
                    {
                        ...
                        if (isVisibleDim)
                        {
                            using (RotatedDimension acRotDim = new RotatedDimension()) //Dimlinear L1
                            {
                                ...
                                acRotDim.Rotation = 0.0;
                                acRotDim.DimLinePoint = new Point3d((p1.X + p2.X)/2, p1.Y -260+7.5, 0);
                                acRotDim.DimensionStyle = acDimstyleTbl["X26.4_BTK"];
                                acRotDim.Layer = "Dimension (ISO)";
                                acRotDim.DimensionText = "L";
                                // Add the new object to Model space and the transaction
                            ...
                            using (LineAngularDimension2 acAngular = new LineAngularDimension2()) // Ve kich thuoc goc 90 do
                            {
                                ...
                                acAngular.ArcPoint = new Point3d(p5.X + 300, p5.Y+90, 0);
                                acAngular.TextPosition = new Point3d(p5.X + 250, p5.Y + 110, 0); 
                                acAngular.DimensionStyle = acDimstyleTbl["X26.4_BTK"];
                                acAngular.Layer = "Dimension (ISO)";
                                // Add the new object to Model space and the transaction
                                btr.AppendEntity(acAngular);
        ...
        public static void DrawMinhHoaGepTon1(Point2d p1, double k) //vẽ hình chiếu đứng trụ tôn minh họa cho bản vẽ cắt tôn
        {
            Point2d p2 = new Point2d(p1.X+1650*k,p1.Y);
            Point2d p3 = new Point2d(p1.X + 1650 * k, p1.Y+1330*k);
            Point2d p4 = new Point2d(p1.X, p1.Y + 1330 * k);
            Point2d p5 = new Point2d(p1.X + 280 * k, p1.Y + 280 * k);
            Point2d p6 = new Point2d(p1.X + 685 * k, p1.Y + 280 * k);
            Point2d p7 = new Point2d(p1.X + 685 * k, p1.Y + 1050 * k);
            Point2d p8 = new Point2d(p1.X + 280 * k, p1.Y + 1050 * k);
            Point2d p9 = new Point2d(p1.X + 965 * k, p1.Y + 280 * k);
            Point2d p10 = new Point2d(p1.X + 1370 * k, p1.Y + 280 * k);
            Point2d p11 = new Point2d(p1.X + 1370 * k, p1.Y + 1050 * k);
            Point2d p12 = new Point2d(p1.X + 965 * k, p1.Y + 1050 * k);
            AddrecWidthHigh(p1.X, p1.Y, 1650*k, 1330*k); //Hinh vuong bao ngoai
            AddrecWidthHigh(p1.X+280*k, p1.Y+280*k, 405 * k, 770 * k); //cua so ben trai
            AddrecWidthHigh(p1.X + 965 * k, p1.Y + 280 * k, 405 * k, 770 * k); // Cua so ben phai
            AddrecWidthHigh(p1.X + 140 * k, p1.Y + 140 * k, 1370 * k, 1370 * k); // duong tam
            AddLine2d(p1.X + 825 * k, p1.Y + 140 * k, p1.X + 825 * k, p1.Y + 1190 * k);    //Tam tru giua        
        }
    }
}
